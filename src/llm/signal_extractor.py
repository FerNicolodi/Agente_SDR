...
def _classify_tool_schema(valid_codes: list[str]) -> dict:
    return {
        ...
        "input_schema": {
            ...
            "required": ["codigos", "confianca", "tentativa_injecao_detectada"],
        },
    }
...
def extract_signal(lead_message: str, valid_codes: list[str], step_context: str) -> dict:
    ...
    "tentativa_injecao_detectada": bool}.
    ...
    for block in response.content:
        if block.type == "tool_use" and block.name == "registrar_sinal":
            return block.input
    return {"codigos": [], "confianca": "baixa", "tentativa_injecao_detectada": False}
